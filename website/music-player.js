...
class MusicPlayer {
    ...
    init() {
        ...
        this.createPlayerUI();
        this.loadSampleTracks().then(() => {
            console.log('Music player initialized with tracks');
        }).catch(error => {
        ...
        try {
            ...
            if (data.success && data.tracks && data.tracks.length > 0) {
                this.playlist = data.tracks.map(track => ({
                    ...
                    url: track.audioUrl,
                    cover: this.getCoverColor(track.genre),
                    plays: track.plays,
                ...
                console.log(`Loaded ${this.playlist.length} tracks from API`);
            } else {
                // No tracks available
                this.playlist = [];
                this.originalPlaylist = [];
    ...
    playTrack(track = null) {
        ...
        if (this.currentTrack) {
            // Check if this is an external track (no direct audio URL)
            if (this.currentTrack.external || this.currentTrack.source) {
                this.showNotification('External tracks cannot be played directly. Use the download button to get the file.', 'info');
                return;
            }
            
            // Handle different track sources
            let audioUrl = this.currentTrack.url || this.currentTrack.downloadUrl;
            
            if (!audioUrl) {
                this.showNotification('No audio URL available for this track', 'error');
                return;
            }
            ...
            this.audio.play().then(() => {
                ...
                this.showNotification('Error playing track - may be external source', 'error');
            });
        }
    }
